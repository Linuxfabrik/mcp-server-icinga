...
1. **The LLM reads its context.** The tool list from step 3 is part of the prompt for this turn. The LLM sees that the `icinga` server exposes a `health_check` tool whose description starts with "Report server status and which backends are configured." That matches the user's request directly.

2. **The LLM emits a structured tool call.** Internally this is the same function-calling mechanism it uses for any tool. The Claude Code client receives a request to call `health_check` on the `icinga` server with no arguments, and forwards the JSON-RPC over stdio:
